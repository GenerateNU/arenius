...
export default function LineItemTable({
  ...
  // Additional state to track if we've loaded data at least once
  const [hasLoadedData, setHasLoadedData] = useState(false);

  // Track if we've ever had data for this table
  useEffect(() => {
    if (!loading && tableData[activeTableData]?.length > 0) {
      setHasLoadedData(true);
    }
  }, [loading, tableData, activeTableData]);

  // Compute if the table is in a loading-like state (initial load or between refreshes)
  const isLoadingOrEmpty = loading || (!hasLoadedData && tableData[activeTableData]?.length === 0);
