...
export default function ExportTransactionsButton() {
    ...
    const exportToXLSX = async () => {
      try {
        ...
        const reconciledData = reconciledResponse.data.line_items.map((line_item: LineItem) => ({
        ...
        const unreconciledData = unreconciledResponse.data.line_items.map((line_item: LineItem) => ({
            Description: line_item.description,
            "Total Amount": line_item.total_amount,
            "Currency Code": line_item.currency_code,
            Contact: line_item.contact_name,
        ...
        const offsetData = offsetResponse.data.line_items.map((line_item: LineItem) => ({
            Description: line_item.description,
            "Total Amount": line_item.total_amount,
            "Currency Code": line_item.currency_code,
            "Emissions Factor": line_item.emission_factor_name,
        ...
        XLSX.utils.book_append_sheet(workbook, reconciledSheet, "Reconciled Transactions");

        const unreconciledSheet = XLSX.utils.json_to_sheet(unreconciledData);
        XLSX.utils.book_append_sheet(workbook, unreconciledSheet, "Unreconciled Transactions");

        const offsetSheet = XLSX.utils.json_to_sheet(offsetData);
        XLSX.utils.book_append_sheet(workbook, offsetSheet, "Carbon Offsets");
        
        const filename = "arenius_transactions_"  + new Date().toISOString().split("T")[0] + ".xlsx";
