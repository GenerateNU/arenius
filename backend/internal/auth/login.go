...
func SupabaseLogin(cfg *config.Supabase, email string, password string) (*SignInResponse, error) {
	...
	if err != nil {
		return nil, errs.BadRequest("failed to execute request")
	}
	...
	if err != nil {
		return nil, errs.BadRequest("failed to read response body")
	}

	// Check if the response was successful
	if resp.StatusCode != http.StatusOK {
		fmt.Printf("failed to login %d, %s", resp.StatusCode, body)
		return nil, errs.BadRequest(fmt.Sprintf("failed to login %d, %s", resp.StatusCode, body))
	}
	...
	if err != nil {
		return nil, errs.BadRequest("failed to parse response body")
	}

	// Make sure response does not contain an error
	if signInResponse.Error != nil {
		return nil, errs.BadRequest(fmt.Sprintf("sign in response error %v", signInResponse.Error))
	}
